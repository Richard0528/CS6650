...
def _lambda_handler(event, context):
    ...
    dynamodb_table = dynamodb.Table('TwinderTable_query')

    db_deserializer = StreamTypeDeserializer()
    ...
    for record in records:
        ...
        if (event_name == 'INSERT') or (event_name == 'MODIFY'):
            if 'NewImage' not in ddb:
                ...
                continue
            doc_fields = db_deserializer.deserialize({'M': ddb['NewImage']})
